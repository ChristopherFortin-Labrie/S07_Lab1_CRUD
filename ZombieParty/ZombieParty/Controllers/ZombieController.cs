...
namespace ZombieParty.Controllers
{
    public class ZombieController : Controller
    {
        ...
        {
            List<Zombie> zombiesList = _baseDonnees.Zombies.Include(z=>z.ZombieType).ToList().OrderBy(z=>z.Name).ToList();

            return View(zombiesList);
        }
        public IActionResult StrongestZombies()
        {
            List<Zombie> zombiesList = _baseDonnees.Zombies.Include(z => z.ZombieType).ToList().OrderBy(z => z.Name).Where(z=>z.Point>=8).ToList();
        ...
        {
            Zombie zombieVM = new Zombie();
            zombieVM = _baseDonnees.Zombies.Find(id);
